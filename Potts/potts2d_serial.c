...
int main(int argc, char *argv[]){
	...
	sus = malloc(r2*sizeof(double));	
		...
		double t;
			...
			t=1/T;
                        ...
                        spec[j] += fabs(E-(e1*e1))*(T*T)/r1;//specific_heat(g,J,T)/r1;
                        sus[j] += fabs(M-(m1*m1))*T/r1;//susceptibility(g,T)/r1;
                        if(k==0) temp[j] = T;
                        T+=0.1;
                        init_grid(g,c,spin,gsl_mt);
                }
	...
	char title[100];
	if(hex==1 && c==0) snprintf(title, 100, "potts2d_stats_hexagonal_serial_hot.txt");
	if(triangle==1 && c==0) snprintf(title, 100, "potts2d_stats_triangular_serial_hot.txt");
	if(sq==1 && c==0) snprintf(title, 100, "potts2d_stats_square_serial_hot.txt");
	if(hex==1 && c==1) snprintf(title, 100, "potts2d_stats_hexagonal_serial_cold.txt");
        if(triangle==1 && c==1) snprintf(title, 100, "potts2d_stats_triangular_serial_cold.txt");
        if(sq==1 && c==1) snprintf(title, 100, "potts2d_stats_square_serial_cold.txt");
